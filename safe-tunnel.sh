...
APP_NAME="Safe-Tunnel"
TG_ID="@SafeVpn5"
VERSION="2.0.0"

GITHUB_REPO="github.com/Safe-Net5/Safe-tunnel"

# MUST match GitHub file name exactly:
SCRIPT_FILENAME="safe-tunnel.sh"
SELF_URL="https://raw.githubusercontent.com/Safe-Net5/Safe-tunnel/main/${SCRIPT_FILENAME}"

PY="/opt/Safe/Safe.py"
PY_URL="https://raw.githubusercontent.com/Safe-Net5/Safe-tunnel/main/Safe.py"

INSTALL_PATH="/usr/local/bin/safe-tunnel"

BASE="/etc/Safe_manager"
CONF="$BASE/profiles"
MAX=10

HC_SCRIPT="/usr/local/bin/Safe-health-check"
HC_CRON_TAG="# SafeTunnelHealthCheck"
...
install_script(){
  ...
  echo "[+] Installed. Run: sudo safe-tunnel" > /dev/tty
}
...
update_script(){
  ...
  if is_installed; then
    ...
    chmod +x "$INSTALL_PATH"
    echo "[+] Updated. Run again: sudo safe-tunnel" > /dev/tty
  else
...
optimize_server(){
  ...
  if sysctl net.ipv4.tcp_available_congestion_control 2>/dev/null | grep -q bbr; then
    ...
    local conf="/etc/sysctl.d/99-Safe-tunnel.conf"
    cat > "$conf" <<'EOF'
# Safe Tunnel - network tuning
